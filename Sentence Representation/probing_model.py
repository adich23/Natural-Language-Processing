...
class ProbingClassifier(models.Model):
    def __init__(self,
        ...
        ## self._pretrained_model.layers[0].layers[0]
        ## self._pretrained_model.layers[1] -> (10k,50) word embeddings
        # self.dense1 = tf.keras.layers.Dense(, activation=tf.nn.relu)
        self._classification_layer = tf.keras.layers.Dense(classes_num)#,input_shape=(self._pretrained_model.layers[0].shape[0],))
    ...
    def call(self, inputs: tf.Tensor, training: bool =False) -> tf.Tensor:
        ...
        # TODO(students): start
        # ...
        # extract out embeddings
        # embedded_tokens = tf.nn.embedding_lookup(self._pretrained_model.layers[1], inputs)
        outputs = self._pretrained_model(inputs,False)
        classification_vector = outputs["layer_representations"][self._layer_num-1]
